...
public class TaskTest {
	...
	private static final int ENDING_AND_BEGINNING_YEAR_DATE = 2016;
	...
	@Test
	public void testTaskHasValidForecast(){
		calendar.set(ENDING_AND_BEGINNING_YEAR_DATE, ENDING_AND_BEGINNING_MONTH_DATE, 1);
		task.setBeginningDate(calendar.getTime());
	...
	@Test
	public void testTaskIsEnding(){
		calendar.set(ENDING_AND_BEGINNING_YEAR_DATE, ENDING_AND_BEGINNING_MONTH_DATE,5);
		task.setEndingDate(calendar.getTime());
		assertTrue(today.getTime().compareTo(task.getEndingDate()) > 0);
	}
	
	@Test 
	public void testTaskHasForecastDate(){
		task.setForecastDate(new Date());
		assertNotNull(task.getForecastDate());
	}
}
